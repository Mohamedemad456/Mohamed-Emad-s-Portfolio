              ...
              <Card delay={index * 0.1} className="h-full flex flex-col text-center items-center">
                ...
                <div className="flex flex-wrap gap-2 mb-6 justify-center">
                  {project.techStack.slice(0, 4).map((tech) => (
                    <span
                      key={tech}
                      className="px-2.5 py-1 bg-slate-50 border border-slate-200 rounded text-xs font-medium text-slate-600 flex items-center justify-center"
                    >
                      {tech}
                    </span>
                  ))}
                  {project.techStack.length > 4 && (
                    <span className="px-2.5 py-1 bg-slate-50 border border-slate-200 rounded text-xs font-medium text-slate-600 flex items-center justify-center">
                      +{project.techStack.length - 4}
                    </span>
                  )}
                </div>
        ...
        {selectedProjectData && (
          ...
          >
            <motion.div
              ...
              className="bg-white rounded-2xl max-w-4xl w-full max-h-[90vh] overflow-y-auto border border-slate-200 shadow-2xl"
            >
              <div className="sticky top-0 bg-white/95 backdrop-blur-md border-b border-slate-100 p-6 flex justify-between items-center z-10">
                ...
                <motion.button
                  ...
                  whileTap={{ scale: 0.9 }}
                  onClick={() => setSelectedProject(null)}
                  className="p-2 text-slate-400 hover:text-cyan-600 hover:bg-cyan-50 rounded-full transition-all"
                >
                  <X size={24} />
                </motion.button>
              </div>
              <div className="p-8 space-y-8">
                <div className="grid md:grid-cols-2 gap-8">
                  <div className="space-y-6">
                    <div>
                      ...
                      <p className="text-slate-600 leading-relaxed">{selectedProjectData.description}</p>
                    </div>
                    ...
                    <div>
                      ...
                      <p className="text-slate-600 leading-relaxed">{selectedProjectData.problem}</p>
                    </div>
                    ...
                    <div>
                      ...
                      <p className="text-slate-600 leading-relaxed">{selectedProjectData.solution}</p>
                    </div>
                  </div>
                    ...
                    <div>
                      ...
                      <div className="flex flex-wrap gap-2">
                        {selectedProjectData.techStack.map((tech) => (
                          <span
                            key={tech}
                            className="px-3 py-1 bg-slate-50 border border-slate-200 rounded-md text-sm font-medium text-slate-600"
                          >
                            {tech}
                          </span>
                        ))}
                ...
                <div>
                  <h3 className="text-sm font-bold uppercase tracking-wider text-cyan-600 mb-4">
                    ...
                    {selectedProjectData.challenges.map((challenge, idx) => (
                      ...
                      >
                        <p className="text-sm text-slate-800 font-semibold mb-2">
                          <span className="text-purple-600">Challenge:</span>{' '}
                          {challenge}
                        </p>
